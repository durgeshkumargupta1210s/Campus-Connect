      ...
      {bookings.map((item,index)=>(
        <div  key={index} className='flex flec-col md:flex-row justify-between bg-primary/8 border border-primary/20 rounded-lg mt-4 p-2 max-w-3xl'>
          <div className='flex flex-col md:flex-row'>
            <img src={item.show.event.poster_path} alt="" className='md:max-w-45 aspect-video h-auto object-bottom rounded' />
            <div className='flex flex-col p-4'>
              <p className='text-lg font-semibold'>{item.show.event.title}</p>
              <p className='text-grey-400 text-sm'>{timeFormat(item.show.event.runtime)}</p>
