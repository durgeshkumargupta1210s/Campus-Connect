...
const AddEvents = () => {
    ...
    const [nowPlayingEvents, setNowPlayingEvents]= useState([]);
    const [selectedEvent,setSelectedEvent]=useState(null);
    const [dateTimeSelection,setDateTimeSelection]=useState({});
    const [dateTimeInput,setDateTimeInput]=useState("");
    const [showPrice,setShowPrice]=useState("");

    const fetchNowPlayingEvents =async () =>{
      setNowPlayingEvents(dummyShowsData)
    };

    // checking that date and time exists and setting them
  ...
  return nowPlayingEvents.length > 0 ? (
    <>
      <Title text1="Add" text2="Shows"/>
      <p className="mt-10 text-lg font-medium">OnGoing Events</p>
      <div className='overflow-x-auto pb-4'>
        <div className="group flex flex-wrap gap-4 mt-4 w-max">
          {nowPlayingEvents.map((event)=>(
            <div key={event.id} onClick={()=>setSelectedEvent(event.id)} className={`relative max-w-40 cursor-pointer transition duration-300 group-hover:opacity-80 hover:-translate-y-1`}>
              <div className='relative rounded-lg overflow-hidden'>
                ...
                <p className='text-gray-300'>{kConverter(event.vote_count)}Votes</p>
                </div>
              </div>
              ...
              <p className='text-gray-400 text-sm'>{event.release_date}</p>
            </div>
          ))}
        </div>
      </div>
